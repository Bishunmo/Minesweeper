...
public class GameMines extends JFrame{
    ...
    GameMines () {
        ...
        canvas.addMouseListener(new MouseAdapter() {
            ...
            public void mouseReleased(MouseEvent e) {
                ...
                if (e.getButton() == MOUSE_BUTTON_LEFT && !bangMine && !youWon){ //left but clicked
    ...
    void openCells(int x, int y) { //recursion to open cells
        if (x < 0 || x > FIELD_SIZE - 1 || y < 0 || y > FIELD_SIZE -1) return; //wring coord
        if (!field[y][x].isNotOpen()) return; //already opened
        field[y][x].open();
        if (field[y][x].getCountBomb() > 0 || bangMine) return; // cell not empty
        for (int dx = -1; dx < 2; dx++)
            for (int dy = -1; dy<2; dy++) openCells(x+dx, y+dy);
    }

    void initField() {  //init of playing field
        ...
        //to count dangerous neighbours around each cell
        for (x = 0; x < FIELD_SIZE; x++)
            for (y = 0; y < FIELD_SIZE; y++)
        ...
        void paint(Graphics g, int x, int y) {
            ...
            if (!isOpen) {
                if ((bangMine || youWon) && isMine) paintBomb(g, x, y, Color.BLACK);
                    ...
                    if (isFlag && !bangMine) paintString(g, SIGN_OF_FLAG, x, y, Color.RED);
                }
            } else
